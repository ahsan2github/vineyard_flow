...
def loadBin(file_in, Nz):
    ...
    with open(file_in,'rb') as f:
        ...
        #print ('file size of %s : %f MB' % (file_in, sz/(1024.0*1024.0)))
        dataRemains = True
        while(dataRemains):
            if b != a:
...
vars=[  'Von','Sc','Co','nnn','pi','model','averaging','fgr',\
        ...
        'partstep','freq','nr','c_flag','Cd', 'row_width', 'row_space', 'n_width',\
        ...
        'emissivity','evolflag','amr_crit','min','max','amr_cnt','h_canopy']
...
def readInput(file_in, vars = vars):
    with open(file_in, mode='r') as f:
        ...
        for x in lines:
                ...
                tempVar = vars.copy() # remember the tempVar becomes empty after the while loop below
                while tempVar:
                    vkey=tempVar.pop()
                    if vkey in lst[0:3]:
                        tf = True
