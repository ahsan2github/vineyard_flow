...
no_vin_rows = 
spacing_bet_rows = 
row_streamwise = 1;  # for rows aligned along the y-direction use 0
...
z_w  = zH_2*l_z;  # For w,uw,vw,wt
z_u  = zH_3*l_z;  # For u,v,T,Pr,Cs
...
fid.close()
plt.figure()
plt.plot(utemp, z_u, '-.og', [min(utemp), max(utemp)], [0.0, 0.0 ], '--r', markerfacecolor = 'None')
plt.xlabel('u (m/s)')
plt.ylabel('z(m)')
plt.title('Log profile')
plt.ylim([-100, 1550])
plt.savefig('uProfile.pdf')
...
fid.close()
plt.figure()
plt.plot(utemp, z_u, '-.ob', [min(utemp), max(utemp)], [0.0, 0.0 ], '--r', markerfacecolor = 'None')
plt.xlabel('v (m/s)')
plt.ylabel('z(m)')
plt.title('v profile')
plt.ylim([-100, 1550])
...
fid.close()
plt.figure()
plt.plot(utemp, z_w, '-.og', [min(utemp), max(utemp)], [0.0, 0.0 ], '--r', markerfacecolor = 'None')
plt.xlabel('w (m/s)')
plt.ylabel('z(m)')
plt.title('w profile')
plt.ylim([-100, 1550])
